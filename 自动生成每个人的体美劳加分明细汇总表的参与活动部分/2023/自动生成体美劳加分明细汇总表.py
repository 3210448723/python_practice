...
# 完整文件
text_path = r"2022-2023学年美育、体育、劳育名单（终）.pdf"
# 部分内容，用于测试
# text_path = r"2022-2023学年美育、体育、劳育名单（终）.pdf"
# 输出目录
...
if __name__ == "__main__":
	# 学号列表：序号,班级,学号,姓名,性别,专业 （除去表头）
	id_list = load_name_list(name_path)[1:]
	# 参与活动记录
	content = read_pdf()
	# 将文本按照活动分割成一个list
	...
	# 总共有几个活动，应该是123
	print(len(content_splitted_list)-1)

	# 标题（第一行）
	header = ['序号', '班级', '学号', '姓名', '性别', '专业', '得分（1次10分）', '参与各活动名称']
	...
	with open(target_path, 'w', encoding='GBK', newline='') as f:
		...
		for id in id_list:
			...
			# 删除GBK编码不支持的字符
			writer.writerow([str(item).replace('\ufeff', '') for item in id])
	# print(id_info)
